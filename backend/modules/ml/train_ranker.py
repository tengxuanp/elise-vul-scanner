...
"""
Train per-family Learning-to-Rank (Stage-B) models from either:
- Evidence logs (attempts/findings), OR
- LTR JSONL generated by synth/gen_synth_datasets.py

What it does
------------
- Parses evidence.jsonl or LTR jsonl and builds labels per (endpoint, payload).
- Labels:
    2 = hard positive (SQL error / boolean or time oracle / JS XSS / external redirect)
    1 = soft positive (big Δlen+Δstatus, large latency delta, or high ML p)      [evidence only]
    0 = negative
- Groups by endpoint (query) and trains per-family LambdaMART (XGBRanker, rank:pairwise).
- Reports NDCG@3 and Hit@1/3/5 on a validation split.
- Saves ranker_{family}.joblib and recommender_meta.json.

Runtime contract
----------------
Inference concatenates:
  [endpoint_feature_vector] + [payload_desc(payload)]
This trainer uses the same recipe:
- Endpoint vector from FeatureExtractor.extract_endpoint_features(...)
- Payload descriptor mirrors backend/modules/recommender._payload_desc
  (For LTR rows lacking raw payloads, we approximate descriptor from payload_id.)
"""
...
def _payload_family_from_body(p: str) -> str:
    s = (p or "").lower()
...
def _payload_family_from_id(pid: Optional[str]) -> str:
    s = (pid or "").lower()
    if s.startswith("xss."): return "xss"
    if s.startswith("sqli."): return "sqli"
    if s.startswith("redir.") or s.startswith("redirect."): return "redirect"
    return "base"
...
def _endpoint_vec(fe: FeatureExtractor, t: Dict[str, Any]) -> List[float]:
    """
    EXACT same payload-agnostic endpoint vector used at inference,
...
def _family_from_record_evidence(rec: Dict[str, Any]) -> str:
    fam = rec.get("payload_family_used")
    if fam:
        return str(fam).lower()
    return _payload_family_from_body(rec.get("payload_string") or rec.get("payload") or "")
...
class Attempt:
    endpoint_id: str
    family: str
    payload_id: Optional[str]   # may be None for evidence logs
    payload_str: Optional[str]  # may be None for LTR rows
    label: int  # 2 hard, 1 soft, 0 negative
    tmeta: Dict[str, Any]       # endpoint meta for features


# ===================== Evidence & LTR parsing / labeling =====================

def _score_label_from_evidence(rec: Dict[str, Any]) -> int:
    """
...
def _looks_like_ltr(r: Dict[str, Any]) -> bool:
    return (
        isinstance(r.get("endpoint"), dict)
        and isinstance(r.get("param"), dict)
        and "label" in r
        and "family" in r
    )
...
def _collect_attempts(data_glob: str, families: Tuple[str, ...]) -> List[Attempt]:
    ...
    if not files:
        raise SystemExit(f"No training files found for glob: {data_glob}")

    out: List[Attempt] = []

    for rec in _iter_jsonl(files):
        # LTR record path
        if _looks_like_ltr(rec):
            fam = str(rec.get("family") or "").lower()
            if fam not in families:
                continue
            ep = rec.get("endpoint") or {}
            prm = rec.get("param") or {}
            # Endpoint meta
            tmeta = {
                "url": ep.get("url"),
                "param": prm.get("name"),
                ...
                "headers": {},  # LTR rows are payload-agnostic; no headers needed for endpoint vec
            }
            ep_id = _endpoint_key(tmeta)
            label = int(rec.get("label", 0))
            # Clamp to {0,1,2}
            label = 2 if label >= 2 else 1 if label == 1 else 0
            out.append(Attempt(
                endpoint_id=ep_id,
                family=fam,
                payload_id=rec.get("payload_id"),
                payload_str=None,  # no raw body in LTR file
                label=label,
                tmeta=tmeta,
            ))
            continue
        ...
        fam = _family_from_record_evidence(rec)
        if fam not in families:
            continue

        lbl = _score_label_from_evidence(rec)
    ...
    for a in out:
        pid_key = a.payload_id if a.payload_id is not None else (a.payload_str or "")
        k = (a.endpoint_id, a.family, pid_key)
...
def _payload_desc_any(a: Attempt) -> List[float]:
    # Prefer real payload body if present; else derive from payload_id
    if a.payload_str:
        return _payload_desc_from_body(a.payload_str)
    return _payload_desc_from_id(a.payload_id)
...
def _build_family_dataset(attempts: List[Attempt], family: str) -> LTRDataset:
    ...
    for ep_id, items in by_ep.items():
        # collapse duplicates per "payload identity" (prefer body string, else payload_id)
        best_by_payload: Dict[str, Attempt] = {}
        for a in items:
            pid_key = a.payload_id if a.payload_id is not None else (a.payload_str or "")
            if pid_key not in best_by_payload or a.label > best_by_payload[pid_key].label:
                best_by_payload[pid_key] = a
        ...
        # Assemble rows
        for it in items:
            X_rows.append(ep_vec + _payload_desc_any(it))
            y_rows.append(float(it.label))  # 0/1/2 gains
            payloads_all.append(it.payload_id or it.payload_str or "")
            endpoint_all.append(ep_id)

        group_sizes.append(len(items))

    if not X_rows:
        raise ValueError(f"No usable training rows for family '{family}'. Check your input logs/files.")
...
def _ndcg_at_k(y_true: List[float], y_score: List[float], k: int = 3) -> float:
    """
    Compute NDCG@k for a single query, robust to queries with < k candidates.
    """
...
def train_one_family(
    ...
    *,
    ...
    if min_g < 2:
        raise ValueError(f"Training groups contain size-1 queries (min={min_g}). Your data is too thin for {family}.")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train per-family LambdaMART/XGBRanker from LTR or evidence jsonl.")
    ap.add_argument("--data-glob", required=True, help="Glob for LTR *.jsonl or evidence *.jsonl")
    ...
    # Collect attempts
    print("[Data] Mining training records...")
    attempts = _collect_attempts(args.data_glob, families)
    print(f"[Data] Collected {len(attempts)} attempts across families {families}")

    if not attempts:
        raise SystemExit("No attempts found. Point --data-glob to LTR *.jsonl or evidence *.jsonl files.")
