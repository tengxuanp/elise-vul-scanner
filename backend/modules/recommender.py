...
class Recommender:
    ...
    New API for provenance:
      recommend_with_meta(...) -> (recommendations, meta_dict)
        - meta_dict has: {"used_path", "family", "applied_threshold", "ranker_score", "top_payload",
                          "scores":[{"payload","raw","prob"}], "model_ids":{...}}
    """
    ...
    def recommend_with_meta(
        ...
        if ranked_with_meta:
            ...
            out = [(p, s) for p, s in ranked if s >= eff_threshold][: max(1, top_n)]
            if out:
        ...
        if self.plugin_fn:
            try:
                ...
                out = [(p, float(prob)) for p, prob, _ in ranked_pairs if prob >= eff_threshold][: max(1, top_n)]
                meta_out.update({
                    "used_path": "plugin",
                    "scores": [{"payload": p, "raw": float(raw), "prob": float(prob)} for p, prob, raw in ranked_pairs],
                })
...
def _extract_hints(feats: Any) -> Dict[str, Any]:
    ...
    if isinstance(feats, dict):
        ...
        ct = feats.get("content_type") or headers.get("content-type")
        return {
            "content_type": ct,
        }
    return {}
