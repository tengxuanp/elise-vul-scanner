    ...
    def reflection_signals(body_text: str, probe: str) -> Dict[str, bool]:
        if not body_text or not probe:
            return {"raw": False, "html_escaped": False, "js_context": False,
                    "attr_context": False, "tag_text_context": False}
...
def _entropy(s: str) -> float:
    ...
    n = float(len(s.encode("utf-8", "ignore")))
    ...
    for tag in soup.find_all():
        ...
        if (payload and (payload in tag_str)) or (enc and (enc in tag_str)):
            ...
            for attr, val in (tag.attrs or {}).items():
                val_str = " ".join(val) if isinstance(val, list) else (val if val is not None else "")
...
class FeatureExtractor:
    ...
    Note: We keep the return shape and indices to avoid breaking your downstream code.
    """
    ...
    def extract_features(
        ...
        try:
            with sync_playwright() as p:
                ...
                context = browser.new_context(ignore_https_errors=True, extra_http_headers=headers or {})
                ...
                try:
                    if method.upper() == "POST":
                        ...
                        # Append/merge our single param simply (builder should ensure de-dup)
                        glue = "&" if "?" in url else "?"
    ...
    def _default_vector(
        ...
        return [
            ...
            1 if _ID_LIKE_RE.search((param or "")) else 0,  # id-like
            ...
            1 if (param or "").lower() in _XSS_PARAM_HINTS else 0,  # xss param hint
            0,                          # entropy bucket
