...
async def enhanced_crawl(
    ...
    allow_pattern_fallback: bool = Query(False, description="Include pattern-generated endpoints (unsafe)")
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        target_url = request.get("target_url", "") or request.get("url", "")
        max_depth = request.get("max_depth", 3)
        max_pages = request.get("max_pages", 12)
        max_links_per_page = request.get("max_links_per_page", 20)
        max_forms_per_page = request.get("max_forms_per_page", 10)
        submit_get_forms = request.get("submit_get_forms", True)
        seed_paths = request.get("seed_paths", None)
        ...
        # Step 2: Add pattern fallback endpoints if requested
        pattern_endpoints = []
        ...
        # Step 4: Limit endpoints if needed
        if max_endpoints and len(all_endpoints) > max_endpoints:
            # Prioritize capture-only endpoints
            captured_count = len(captured_endpoints)
            if captured_count >= max_endpoints:
                all_endpoints = captured_endpoints[:max_endpoints]
            else:
                remaining = max_endpoints - captured_count
                all_endpoints = captured_endpoints + pattern_endpoints[:remaining]
            logger.info(f"📊 Limited to {max_endpoints} endpoints")
        
        # Step 5: Determine if ready for fuzzing
        ...
        response = {
            ...
            "discovered_endpoints": len(captured_endpoints),  # Only count capture-only
            "endpoints": all_endpoints,
            "enhanced_ml": True,
            "ready_for_fuzzing": ready_for_fuzzing,
            "capture_only": not allow_pattern_fallback,
            "pattern_fallback_used": allow_pattern_fallback
        }
        
        # Add helpful note if no endpoints discovered
        if len(captured_endpoints) == 0:
            response["note"] = "No endpoints observed. Check if the app is SPA and requires user interaction. Increase max_pages or enable scripted clicks."
            logger.warning("⚠️ No endpoints discovered - providing guidance to user")
        
        logger.info(f"🎉 Enhanced Crawl: {len(captured_endpoints)} capture-only + {len(pattern_endpoints)} pattern endpoints")
        fuzzing_ready_count = sum(1 for ep in all_endpoints if ep.get("param_names") and len(ep.get("param_names", [])) > 0 and ep.get("status") and 200 <= ep.get("status", 0) < 400)
        logger.info(f"🎯 Ready for fuzzing: {ready_for_fuzzing} ({fuzzing_ready_count} endpoints with params)")
        return response
