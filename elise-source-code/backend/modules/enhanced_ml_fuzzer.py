...
class EnhancedMLFuzzer:
    ...
    def fuzz_target(self, target: FuzzTarget, top_k: int = 5) -> List[FuzzResult]:
        ...
        for payload in payloads:
            ...
            # Use ML prediction for vulnerability classification - NO FALLBACKS
            if not hasattr(target, 'ml_prediction') or not hasattr(target, 'top_family'):
                logger.error(f"❌ Target {target.url} missing ML prediction - cannot classify vulnerability")
                vulnerability = None
            else:
                # Use ML prediction to determine vulnerability type
                top_family = target.top_family
                family_dist = target.family_dist
                
                # Map ML family to VulnerabilityType
                family_mapping = {
                ...
                if top_family in family_mapping:
                    vuln_type = family_mapping[top_family]
                    cvss_metrics = self.cvss_templates[vuln_type]
                    confidence_score = family_dist.get(top_family, 0.0)
                    ...
                    logger.info(f"🧠 ML-based classification: {top_family} (confidence: {confidence_score:.3f})")
                else:
                    logger.error(f"❌ Unknown ML family: {top_family}")
                    vulnerability = None
            
            # Create result with payload recommendation evidence
            result = FuzzResult(
                target=target,
                payload=payload,
            ...
            # Add payload recommendation evidence if available
            if hasattr(target, 'recommendation_details'):
                for rec in target.recommendation_details:
                    if rec.payload == payload:
                        result.recommendation_why = rec.why
                        result.payload_score = rec.score
                        result.payload_placement = rec.placement
                        break
    ...
    def _generate_payloads(self, target: FuzzTarget, top_k: int) -> List[str]:
        """Generate payloads using deterministic recommender - NO FALLBACK HEURISTICS"""
        payloads = []
        
        # Check if target has ML prediction data
        if not hasattr(target, 'ml_prediction') or not hasattr(target, 'top_family'):
            logger.error(f"❌ Target {target.url} missing ML prediction data - cannot generate payloads")
            return []
        
        # Use ML prediction to select vulnerability family
        top_family = target.top_family
        family_dist = target.family_dist
        
        logger.info(f"🧠 Using ML prediction for {target.url}: top_family={top_family}, dist={family_dist}")
        
        # Import and use deterministic payload recommender
        try:
            from modules.payload_recommender import rank_payloads
            
            # Build context for payload ranking
            context = {
                "param_name": target.param,
                "param_location": "query" if "?" in target.url else "form",
                "param_value": "test",  # Placeholder for context analysis
                "url": target.url,
                "method": target.method
            }
            
            # Get ranked payloads using deterministic recommender
            recommendations = rank_payloads(top_family, family_dist, context)
            
            # Store recommendation details for evidence
            target.recommendation_details = recommendations[:top_k]
            
            # Extract payload strings from recommendations
            for rec in recommendations[:top_k]:
                payloads.append(rec.payload)
                logger.info(f"🎯 Selected payload: {rec.payload[:30]}... (score: {rec.score:.2f}, why: {rec.why})")
            
        except ImportError as e:
            logger.error(f"❌ Failed to import payload recommender: {e}")
            return []
        except Exception as e:
            logger.error(f"❌ Failed to rank payloads: {e}")
            return []
        
        logger.info(f"🎯 Generated {len(payloads)} payloads for family {top_family} using deterministic ranking")
        return payloads
